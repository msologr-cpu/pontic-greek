...
носитель языка, ровно на согласованную величину, и ни к каким другим.
"""
import sys
from fontTools.ttLib import TTFont

ASKED = 'ξζχΣΞΖΧ'      # семь букв из замечания (после уточнения)
EXCLUDED = 'ψΨ'         # носитель языка: «и так нормальные»
...
def main(before, after, expected=15):
    ...
    for ch in WATCH:
        ...
        if ch in ASKED:
            good = (d == expected)
            note = f"просил носитель языка → ожидаем +{expected}"
        else:
            good = (d == 0)
