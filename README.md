...
> **Текущая версия (единая для всех платформ):** **v5.1** (Август 2026)
...
- **PonticKeyboard_v5.1.exe** — Единая клавиша `/` *(рекомендуется)*
- **PonticKeyboard_v5.1_3keys.exe** — Раздельные 3 клавиши `,` `.` `/`
- **PonticWordRestore.exe** — 🔧 лечит апостроф в Word после версий 4.0–5.0
  *(запустить один раз, если апостроф печатается прямым штрихом)*
- 📖 [Подробная инструкция для Windows](ИНСТРУКЦИЯ_WINDOWS.md)

### 🆕 Что нового в 5.1

- **Исправлен апостроф в Word.** Версии 4.0–5.0 по ошибке выключали в Word
  «умные кавычки» — глобально и навсегда. Из-за этого клавиша апострофа
  начинала печатать прямой штрих `'` вместо правильных `‘ ’` во всех
  документах, даже без запущенной понтийской раскладки. Теперь эта настройка
  не трогается, а если была испорчена — восстанавливается автоматически.
- **Гачек сдвинут правее** над `ξ ζ χ` и `Σ Ξ Ζ Χ` — оптическая поправка
  по замечанию носителя языка. `ψ` и `Ψ` намеренно оставлены как были.
